...
//    P1OUT &= ~BIT3;
    ...
    // Make timer input P3.5 for electrical compatibility with everything else
    // P3.4 also needs to be an input
    P3DIR &= ~(BIT4 | BIT5);
    P3REN |= BIT4 | BIT5;
    P3OUT &= ~(BIT4 | BIT5);

    //P1.3 also needs to be input for electrical compatibility with everything else
    P1DIR &= ~(BIT3);
    P1REN |= BIT3;
    P1OUT &= ~BIT3;
    ...
    TA0CCTL0 = CCIE;
    TA0CCR0 = 1000;                         // 10 kHz clock -> 0.05 second period
    TA0CTL = TASSEL__ACLK | MC__UP;

    // ----- Configure ADC -----
    REFCTL0 |= REFVSEL_0;                   // Internal reference 1.2V
...
//    if(state == WAIT && (P1IN & BIT3)){             // If P1.3 high, we've received signal to take a sample
    if(state == WAIT && (P3IN & BIT5)){             // P1.3 -> P3.5
        state = SENSE;
...
//    }else if(state == FILTER && (~P1IN & BIT3)){    // If P1.3 goes low after we've completed a sample, get ready to take another
    }else if(state == FILTER && (~P3IN & BIT5)){    // P1.3 -> P3.5
        state = WAIT;
        __bic_SR_register_on_exit(LPM3_bits); // Begin running
    }
}
